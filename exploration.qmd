...
Look at using Rcpp code to pass in functions. This may be good as the cpp will not call the R function. 
...
u <- lapply(1:2, function(i) S(paste0("u", i)))
p <- lapply(1:5, function(i) S(paste0("p", i)))
t <- S("t")

du <- f(u, p, t) |>
  vapply(as.character, character(1))

# This takes in a symbolic version of the system, builds it in cpp, and evaluates it given , u ,t and the parameters

result <- symbolicSystemEval(du, U, c(1, 2), 1.0, as.numeric(py$p_star))
result
print(f(c(1, 2), as.numeric(py$p_star), 1))
```